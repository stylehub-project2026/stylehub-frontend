  ...
  return (
    <div onClick={() => navigate(`/product/${p.id}`)}
      ...
      <div style={{ position: "relative", overflow: "hidden", aspectRatio: SZ.gridCardRatio, background: "#f0ece6" }}>
        ...
        {p.oldPrice && <div style={{ position: "absolute", top: ".7rem", left: ".7rem", background: "var(--red)", color: "#fff", fontSize: ".52rem", letterSpacing: ".1em", padding: ".2rem .55rem", fontWeight: 700, borderRadius: 3 }}>SALE</div>}
      ...
      <div style={{ padding: ".55rem .65rem" }}>
        <div style={{ fontSize: ".52rem", letterSpacing: ".15em", textTransform: "uppercase", color: "var(--warm)", marginBottom: ".15rem" }}>27</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
      ...
      {/* ════════════════════════════════
          1. HERO BANNER
      ════════════════════════════════ */}
      <section className="b27-hero" style={{
        ...
        <div className="b27-hero-inner" style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", gap: "4rem", padding: "0 6%", width: "100%" }}>

          {/* LOGO */}
          <div className="b27-hero-logo" style={{ flexShrink: 0, width: SZ.heroLogoSize, height: SZ.heroLogoSize, borderRadius: "50%", overflow: "hidden", background: "#f8f6f2", border: "1.5px solid rgba(26,26,24,.12)", boxShadow: "0 8px 32px rgba(26,26,24,.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <img src={BRAND.logo} alt={BRAND.name} style={{ width: "100%", height: "100%", objectFit: "contain" }}
              onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
            <div style={{ display: "none", width: "100%", height: "100%", alignItems: "center", justifyContent: "center", fontFamily: "'Cormorant Garamond',serif", fontSize: "4rem", fontWeight: 600, color: BRAND.accentColor }}>27</div>
          </div>

          {/* TEXT */}
          <div className="b27-hero-text" style={{ maxWidth: 500 }}>
            <div style={{ fontSize: ".58rem", letterSpacing: ".3em", textTransform: "uppercase", color: "var(--warm)", marginBottom: ".6rem" }}>StyleHub · Brand</div>
            <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(2.6rem,4vw,3.6rem)", fontWeight: 400, lineHeight: 1.1, marginBottom: "1rem", color: "var(--dark)" }}>{BRAND.fullName}</h1>
            <p className="b27-hero-desc" style={{ fontSize: ".90rem", lineHeight: 1.85, color: "#555252", marginBottom: "1.8rem", maxWidth: 440 }}>{BRAND.desc}</p>
            <button onClick={scrollGrid} className="b27-cta-btn">Shop Now</button>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#D8D4CE", padding: "4rem 6%", borderTop: "1px solid rgba(26,26,24,.08)", borderBottom: "1px solid rgba(26,26,24,.08)" }}>
        <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(1.6rem,2.5vw,2.2rem)", fontWeight: 400, textAlign: "center", marginBottom: "2.5rem", letterSpacing: ".04em" }}>
          Shop by {BRAND.fullName}
        </h2>
        <div className="b27-shop-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem" }}>
      ...
      <section style={{ background: "#ffffff", padding: "5rem 6%", borderBottom: "2px solid rgba(26,26,24,.12)" }}>
        <div className="b27-popular-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>
          <div className="reveal" ref={addRef}>
            <div style={{ fontSize: ".58rem", letterSpacing: ".3em", textTransform: "uppercase", color: "var(--warm)", marginBottom: "1rem" }}>27 Collection</div>
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(1.9rem,3vw,2.8rem)", fontWeight: 400, lineHeight: 1.2, marginBottom: "1.2rem" }}>Bold. Minimal. Cairo.</h2>
            <p style={{ fontSize: ".86rem", lineHeight: 1.85, color: "#555252", marginBottom: "2rem", maxWidth: 400 }}>{BRAND.desc}</p>
            <button onClick={scrollGrid} className="b27-cta-btn">Shop Now</button>
          </div>

          {/* decorative blob */}
          <div className="b27-popular-blob" style={{ position: "relative", minHeight: "300px" }}>
            <div style={{ position: "absolute", right: "-6%", top: "-22%", width: "52%", height: "144%", background: BRAND.accentColor, borderRadius: "60% 0 0 60%", zIndex: 0 }} />
          </div>
        </div>
      </section>
      ...
      {/* ════════════════════════════════
          4. ALL PRODUCTS
      ════════════════════════════════ */}
      <div className="b27-products-layout" style={{ display: "flex", gap: "2.5rem", padding: "3rem 6%", alignItems: "flex-start", background: "var(--cream)" }}>

        {/* MOBILE FILTER TOGGLE */}
        <button className="b27-filter-toggle" onClick={() => setSidebarOpen(o => !o)}>
          <span>☰</span> {sidebarOpen ? "Hide Filters" : "Show Filters"}
          {hasActiveFilters && <span className="b27-filter-badge">●</span>}
        </button>

        {/* SIDEBAR */}
        <div className={`b27-sidebar${sidebarOpen ? " sidebar-open" : ""}`} style={{ width: 185, flexShrink: 0, position: "sticky", top: "70px" }}>
        ...
        {/* GRID */}
        <div id="b27-grid" style={{ flex: 1, minWidth: 0 }}>
          ...
          {filtered.length === 0
            ...
            : <div className="b27-product-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.4rem" }}>
              {paginated.map(p => <PCard key={p.id} p={p} wish={wish} toggleWish={toggleWish} />)}
            </div>
          }

          {totalPages > 1 && (
            <div className="b27-pagination-row" style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: ".45rem", padding: "2.5rem 0" }}>
...
// ─── PAGE CSS ───
const PAGE_CSS = `
.filter-label { font-size:.6rem; letter-spacing:.2em; text-transform:uppercase; font-weight:600; margin-bottom:.65rem; color:var(--dark); }
.b27-cta-btn { background:#92A079; color:#fff; border:none; padding:.65rem 1.8rem; font-size:.68rem; letter-spacing:.14em; text-transform:uppercase; font-weight:600; cursor:pointer; font-family:'DM Sans',sans-serif; border-radius:3px; transition:background .2s; }
.b27-cta-btn:hover { background:var(--dark); }

/* ── Mobile filter toggle — hidden on desktop ── */
.b27-filter-toggle { display:none; }

/* ════════════════════════════
   TABLET  (≤1024px)
════════════════════════════ */
@media (max-width:1024px) {
  .b27-shop-grid { grid-template-columns:repeat(2,1fr) !important; }
  .b27-product-grid { grid-template-columns:repeat(2,1fr) !important; }
  .b27-hero-inner { gap:2.5rem !important; }
}
...
@media (max-width:768px) {
  ...
  .b27-hero-logo { width:110px !important; height:110px !important; }
  .b27-hero-text { max-width:100% !important; }
  .b27-hero-desc { display:none !important; }

  /* Shop grid */
  .b27-shop-grid { grid-template-columns:repeat(2,1fr) !important; gap:0.75rem !important; }

  /* Popular section */
  .b27-popular-grid { grid-template-columns:1fr !important; gap:0 !important; }
  .b27-popular-blob { display:none !important; }

  /* Products layout */
  .b27-products-layout { flex-direction:column !important; padding:1.2rem 4% !important; gap:0 !important; }
  ...
  .b27-filter-badge { color:#92A079; font-size:.9rem; line-height:1; }
  ...
  .b27-sidebar.sidebar-open { display:block !important; }

  /* Product grid */
  .b27-product-grid { grid-template-columns:repeat(2,1fr) !important; gap:0.8rem !important; }

  /* Pagination */
  .b27-pagination-row { flex-wrap:wrap !important; gap:.3rem !important; padding:1.5rem 0 !important; }
}
...
@media (max-width:480px) {
  .b27-shop-grid { grid-template-columns:repeat(2,1fr) !important; gap:0.5rem !important; }
  .b27-product-grid { grid-template-columns:repeat(2,1fr) !important; gap:0.6rem !important; }
  .b27-hero { margin:0.4rem !important; }
  .b27-hero-logo { width:90px !important; height:90px !important; }
}
`;
