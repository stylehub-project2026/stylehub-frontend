...
export default function TwentySevenBrand({ cart, wish = [], setWish }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API}/products?brand=27&limit=100`)
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
      ...
      {/* HERO */}
      <section style={{ position: "relative", height: SZ.heroBanner, overflow: "hidden", background: "#ffffff", display: "flex", alignItems: "center", margin: "1.5rem", borderRadius: 10, border: "2px solid rgba(26,26,24,.2)", boxShadow: "0 4px 24px rgba(26,26,24,.07)" }}>
        <img src={BRAND.heroBg} alt="" aria-hidden style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: .08, borderRadius: 10 }} onError={e => e.target.style.display = "none"} />
        ...
        <div style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", gap: "4rem", padding: "0 6%", width: "100%" }}>
          <div style={{ flexShrink: 0, width: SZ.heroLogoSize, height: SZ.heroLogoSize, borderRadius: "50%", overflow: "hidden", background: "#f8f6f2", border: "1.5px solid rgba(26,26,24,.12)", boxShadow: "0 8px 32px rgba(26,26,24,.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <img src={BRAND.logo} alt={BRAND.name} style={{ width: "100%", height: "100%", objectFit: "contain" }} onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
          ...
          <div style={{ maxWidth: 500 }}>
            ...
            <p style={{ fontSize: ".90rem", lineHeight: 1.85, color: "#555252", marginBottom: "1.8rem", maxWidth: 440 }}>{BRAND.desc}</p>
      ...
      <section style={{ background: "#D8D4CE", padding: "4rem 6%", borderTop: "1px solid rgba(26,26,24,.08)", borderBottom: "1px solid rgba(26,26,24,.08)" }}>
        <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(1.6rem,2.5vw,2.2rem)", fontWeight: 400, textAlign: "center", marginBottom: "2.5rem", letterSpacing: ".04em" }}>Shop by {BRAND.fullName}</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem" }}>
          {allProducts.slice(0, 4).map((p, i) => (
            <div key={p.id} className={`reveal d${i + 1}`} ref={addRef} style={{ cursor: "pointer", textAlign: "center" }} onClick={() => navigate(`/product/${p.id}`)}>
        ...
        <div style={{ width: 185, flexShrink: 0, position: "sticky", top: "70px" }}>
          ...
          <FilterSection title="Size">
            <div style={{ display: "flex", flexWrap: "wrap", gap: ".32rem" }}>
              {ALL_SIZES.map(s => (
                <button key={s} onClick={() => toggleSize(s)} style={{ padding: ".26rem .52rem", fontSize: ".63rem", border: `1.5px solid ${selSizes === s ? "var(--dark)" : "var(--border)"}`, background: selSizes === s ? "var(--dark)" : "transparent", color: selSizes === s ? "#fff" : "var(--dark)", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s", minWidth: 32, borderRadius: 3 }}>{s}</button>
              ))}
            </div>
          </FilterSection>
          <FilterSection title="Color">
            <div style={{ display: "flex", flexWrap: "wrap", gap: ".4rem" }}>
              {ALL_COLORS.map((c, i) => (
                <div key={i} onClick={() => toggleColor(c)} style={{ width: 22, height: 22, borderRadius: "50%", background: c, cursor: "pointer", border: (c === "#ffffffff" || c === "#fff") ? "1.5px solid var(--border)" : "2px solid transparent", boxShadow: selColors === c ? "0 0 0 2.5px var(--dark)" : "none", transform: selColors === c ? "scale(1.2)" : "none", transition: "all .2s" }} />
              ))}
            </div>
          </FilterSection>
          {(selSizes || selColors || selType !== "all") && (
            <button onClick={() => { setSelSizes(null); setSelColors(null); setSelType("all"); setPage(1); }} style={{ fontSize: ".61rem", letterSpacing: ".1em", textTransform: "uppercase", background: "none", border: "1px solid var(--border)", padding: ".36rem .8rem", cursor: "pointer", color: "var(--warm)", fontFamily: "'DM Sans',sans-serif", width: "100%", borderRadius: 3 }}>Clear Filters</button>
          )}
        </div>

        <div id="b27-grid" style={{ flex: 1 }}>
