  ...
  return (
    <div onClick={() => navigate(`/product/${p.id}`)}
      ...
      <div style={{ padding: ".55rem .65rem" }}>
        <div style={{ fontSize: ".52rem", letterSpacing: ".15em", textTransform: "uppercase", color: "var(--warm)", marginBottom: ".15rem" }}>Antika</div>
...
// ─── MAIN PAGE ───
export default function AntikaBrand({ cart, wish = [], setWish }) {
  ...
  const scrollGrid = () => document.getElementById("antika-grid")?.scrollIntoView({ behavior: "smooth" });

  useEffect(() => {
    fetch(`${API}/products?brand=Antika&limit=100`)
      .then(r => r.json())
      .then(data => {
        const prods = (data.data?.products || []).map(p => ({
          id: p._id, _id: p._id, name: p.name, brand: "Antika",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)" }}>
      ...
      {/* ════════════════════════════════
          1. HERO BANNER
      ════════════════════════════════ */}
      <section className="antika-hero" style={{
        ...
        <div className="antika-hero-inner" style={{ position: "relative", zIndex: 2, display: "flex", alignItems: "center", gap: "4rem", padding: "0 6%", width: "100%" }}>

          {/* LOGO */}
          <div className="antika-hero-logo" style={{ flexShrink: 0, width: SZ.heroLogoSize, height: SZ.heroLogoSize, borderRadius: "50%", overflow: "hidden", background: "#f8f6f2", border: "1.5px solid rgba(26,26,24,.12)", boxShadow: "0 8px 32px rgba(26,26,24,.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          {/* TEXT */}
          <div className="antika-hero-text" style={{ maxWidth: 500 }}>
            ...
            <p className="antika-hero-desc" style={{ fontSize: ".90rem", lineHeight: 1.85, color: "#555252", marginBottom: "1.8rem", maxWidth: 440 }}>
              {BRAND.desc}
            </p>
            <button onClick={scrollGrid} className="antika-cta-btn">Shop Now</button>
          </div>
        </div>
      </section>

      {/* ════════════════════════════════
          2. SHOP BY ANTIKA  (grey bg)
      ════════════════════════════════ */}
      <section style={{ background: "#D8D4CE", padding: "4rem 6%", borderTop: "1px solid rgba(26,26,24,.08)", borderBottom: "1px solid rgba(26,26,24,.08)" }}>
        ...
        <div className="antika-shop-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem" }}>
      ...
      <section style={{ background: "#ffffff", padding: "5rem 6%", borderBottom: "2px solid rgba(26,26,24,.12)" }}>
        <div className="antika-popular-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>

          {/* LEFT TEXT */}
          <div className="reveal" ref={addRef}>
            <div style={{ fontSize: ".58rem", letterSpacing: ".3em", textTransform: "uppercase", color: "var(--warm)", marginBottom: "1rem" }}>Antika Collection</div>
            ...
            <button onClick={scrollGrid} className="antika-cta-btn">Click Here</button>
          </div>

          {/* RIGHT — decorative dark blob */}
          <div className="antika-popular-blob" style={{ position: "relative", minHeight: "300px" }}>
      ...
      ════════════════════════════════ */}
      <div className="antika-products-layout" style={{ display: "flex", gap: "2.5rem", padding: "3rem 6%", alignItems: "flex-start", background: "var(--cream)" }}>

        {/* MOBILE FILTER TOGGLE */}
        <button
          className="antika-filter-toggle"
          onClick={() => setSidebarOpen(o => !o)}
        >
          <span>☰</span> {sidebarOpen ? "Hide Filters" : "Show Filters"}
          {hasActiveFilters && <span className="antika-filter-badge">●</span>}
        </button>

        {/* SIDEBAR */}
        <div className={`antika-sidebar${sidebarOpen ? " sidebar-open" : ""}`} style={{ width: 185, flexShrink: 0, position: "sticky", top: "70px" }}>
        ...
        {/* GRID */}
        <div id="antika-grid" style={{ flex: 1, minWidth: 0 }}>
          ...
          {filtered.length === 0
            ...
            : <div className="antika-product-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1.4rem" }}>
          ...
          {/* PAGINATION */}
          {totalPages > 1 && (
            <div className="antika-pagination-row" style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: ".45rem", padding: "2.5rem 0" }}>
...
.antika-cta-btn:hover { background:#92A079; }

/* ── Mobile filter toggle — hidden on desktop ── */
.antika-filter-toggle { display:none; }
...
@media (max-width:768px) {
  ...
  .antika-hero-text { max-width: 100% !important; }
  .antika-hero-desc { display: none !important; }
  ...
  .antika-popular-blob { display: none !important; }

  /* Products layout — stack sidebar above grid */
  .antika-products-layout {
    flex-direction: column !important;
  ...
  .antika-sidebar.sidebar-open { display: block !important; }
