...
const API_BASE = "https://stylehub-backend-ten.vercel.app/api";
            ...
            {searchResults.map(p => (
              <div key={p._id} onClick={() => { navigate(`/product/${p._id}`); closeSearch(); }}
                ...
                <div style={{ width: 42, height: 52, background: "#f0ece6", flexShrink: 0, overflow: "hidden", borderRadius: 4 }}>
                  {p.images?.[0] && <img src={`https://stylehub-backend-ten.vercel.app${p.images[0]}`} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => e.target.style.display = "none"} />}
                </div>
                <div style={{ flex: 1 }}>
